...
> 当 docs 文件之间冲突时，优先级为：产品方案 > UI 设计 > 实施指南 > AGENTS.md。
...
**例外**：跨 Phase 的架构纪律、业务规则、文件命名、技术栈选型等已确定的内容可以在产品方案、AGENTS.md、UI 设计文档中提前固化。规则的边界是"是否依赖前一 Phase 的实际产出来决定"——依赖则等，不依赖则可以先写。
