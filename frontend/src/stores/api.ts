/** API client — 全部请求打到 localhost 上的工作台后端。 */
...
export interface PoListItem {
  ...
  chain_segments: { seller: string; buyer: string }[]
  line_count: number
...
export interface EditRequest {
  ...
  field: string
  value: unknown
  // no po_no here — it's on the URL path, set by caller
}

export const api = {
  health: (): Promise<{ status: string }> => request("GET", "/health"),

  openSession: (base_file: string): Promise<SessionData> =>
  ...
  exportDocuments: (req: DryRunRequest): Promise<DryRunResult> =>
    request("POST", "/export", req),
};
