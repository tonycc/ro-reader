...
/**
 * 修复 PIN 输入弹窗。
 * 保存 override 前弹出，校验通过才允许写入。
 * 软管控：仅约束界面内的保存操作。
 */

const props = defineProps<{
  open: boolean;
  /** 本次会话内剩余可尝试次数（-1 表示未限制展示） */
  attemptsLeft: number;
  /** 锁定时长（秒），>0 表示锁定中 */
  lockSeconds: number;
}>();
...
function onConfirmFail(message: string) {
  error.value = message;
  pin.value = "";
  inputRef.value?.focus();
}

defineExpose({ onConfirmFail });
</script>
  ...
  <Teleport to="body">
    <div v-if="open" class="pin-overlay" @click.self="emit('cancel')">
      <div class="pin-dialog" role="dialog" aria-modal="true" aria-labelledby="pin-title">
        ...
        <template v-if="lockSeconds > 0">
          <div class="pin-locked">
            <span class="lock-icon">🔒</span>
            <p>尝试次数过多，请 {{ Math.ceil(lockSeconds / 60) }} 分钟后再试</p>
          </div>
        </template>
        <template v-else>
          <input
          ...
          <p v-if="attemptsLeft >= 0 && attemptsLeft < 5" class="pin-attempts">
            还可尝试 {{ attemptsLeft }} 次
          </p>
        </template>
